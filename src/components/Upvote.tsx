...
const Upvote = React.forwardRef<HTMLDivElement, UpvoteProps>(({ data, saved }, ref) => {
  ...
  const [viewResult, setViewResult] = useState<boolean>(data?.userVotedOptionId ? true : false);
  ...
  const { data: pollResultData, refetch } = usePollResultQuery({ id: data?._id || '' }, { skip: data?.uploadType !== POST_TYPE.POLL || roleName === 'parent' });
  ...
  const isPDF = (url: string) => url.split(".").pop()?.toLowerCase() === "pdf";
  const isVideo = (url: string) =>
    ["mp4", "webm", "ogg"].includes(url.split(".").pop()?.toLowerCase() || "");
    ...
    <div ref={ref} className="study_card_wrp">
      ...
      >
        ...
        {isUser ? (
          ...
          <div className="filter_menubar">
            <Menu className="filter_menubar" anchorEl={anchorEl} open={open} onClose={handleClose}>
              <MenuItem >
                <RWebShare
                  data={{
              ...
              <MenuItem onClick={() => token ? setOpenReport(true) : setOpen2}>
                <figure>
                  <img src={`/static/images/shield.png`} alt="svg" />
                  <figcaption>{"Report Learner"}</figcaption>
                </figure>
              </MenuItem>
              <MenuItem onClick={() => { if (token) { setOpenHideModal(true); handleClose() } else { setOpen2(true) } }}>
                <figure>
                  <img src={`/static/images/approved.png`} alt="svg" />
                  <figcaption>{"Mute Learner"}</figcaption>
                </figure>
              </MenuItem>
              <MenuItem onClick={() => { if (token) { notInteresed(); handleClose() } else { setOpen2(true) } }}>
                <figure>
              ...
              <MenuItem onClick={() => {
                if (token) { setOpenDontModal(true); handleClose() } else {
                  setOpen2(true)
                }
              }} >
                <figure>
      ...
      >
        ...
        {data?.uploadType == POST_TYPE.POLL ? (
          <div className="poll_options">
            <h2>{data?.question || ''}</h2>
            {(data?.userVotedOptionId && roleName == 'parent') || (!data?.userVotedOptionId && viewResult) || (roleName == 'tutor' && viewResult) ? (
              <ul>
                {
                  roleName == 'parent' ? (
                    data?.pollOptions?.map((option: any, index: number) => (
                      <li className="home_li_progress" key={index}>
                        <p>  <span>{option?.option}</span> {option?.votes ? ((option?.votes / data?.votesCount) * 100)?.toFixed(1) : 0}%</p>
                        {!option?.votes ? (
                          ...
                          <LinearProgress variant="determinate" value={option?.votes ? ((option?.votes / data?.votesCount) * 100) : 0} sx={{
                            height: 10,
                            borderRadius: 5,
                    ...
                    pollResultData?.data?.results?.map((option: any, index: number) => (
                      <li className="home_li_progress" key={index}>
                        <p>  <span>{option?.option}</span> {Math.round(option?.percentage) || 0}%</p>
                        {option?.percentage && option?.percentage == '0.00' ? (
                          <LinearProgress variant="determinate" value={100} sx={{
                            height: 10,
                            borderRadius: 5,
                          ...
                          <LinearProgress variant="determinate" value={Math.round(option?.percentage) || 0} sx={{
                            height: 10,
                            borderRadius: 5,
              ...
              <ul className="poll_options_list">
                {data?.pollOptions?.map((option: any, index: number) => (
                  <li key={index}>
                    <Box onClick={() => token ? addVoteFunction(option?._id) : setOpen2(true)} sx={{ display: 'flex', alignItems: 'center' }}>
                      <span>{option?.option} ({option.votes || 0} votes)</span>
                    </Box>
                  </li>
                ))}
            ...
            {data?.userVotedOptionId && roleName == 'parent' ? null :
              viewResult ? <p onClick={() => setViewResult(false)}>{pollResultData?.data?.totalVotes || 0}{' '}{pollResultData?.data?.totalVotes == 1 ? "vote" : "votes"} . Hide Results</p> :
      ...
      <div className="social_footer">
        <div className="row">
          <ul className="rt__s">
            <li>
              <figure onClick={() => token ? handleEngagement(Engagements.LIKE) : setOpen2(true)}>
                <img
                  src={
                    data?.isLike
            ...
            <li>
              <figure onClick={() => token ? handleEngagement(Engagements.UPVOTE) : setOpen2(true)}>
                <img
            ...
            <li>
              <figure onClick={() => token ? handleEngagement(Engagements.DOWNVOTE) : setOpen2(true)}>
                <img
                  style={{ width: "18px", height: "18px" }}
                  src={
                    data?.isDownvote
            ...
            {isUser || data?.createdBy == UPVOTE_ROLE.PARENT ? null : (
              <li onClick={() => token ? followTutorFunc() : setOpen2(true)}>
                <figure>
                  <img
                    src={
                      data?.isFollowing
            ...
            <li onClick={() => token && roleName=='tutor' ? setOpenGift1(true): token && roleName=='parent' ? setOpenGift(true) : setOpen2(true)}>
              <figure>
                <img src={`/static/images/gift_bk.png`} alt="svg" />
            ...
            {isUser ? null : (
              <li onClick={() => setOpenReport(true)}>
                <Tooltip title="Report Forum">
                  <figure>
                    <img src={`/static/images/problem.png`} alt="svg" />
                  </figure>
                </Tooltip>
              </li>
            )} */}
          ...
          <div className="comment_section">
            ...
            <div className="send_group_row">
              <TextField
                inputRef={replyRef}
                placeholder={
                  reply.isReply ? `reply to ${reply.userName}.. ` : "Add comment"
                }
                value={comment}
                onChange={(e) => {
              ...
              <span
                className="gift_btn"
                onClick={() => (reply.isReply ? AddCommentReply() : AddComment())}
              >
